                        ...
                        <table class="min-w-max w-full text-base">
                            <tbody>
                                @forelse($news as $item)
                                    <tr>
                                        <td id="news-date" class="text-xs pt-2">{{$item->created_at->format('Y年m月d日')}} @if($item->target_user == auth()->user()->id) <span class="px-2 py-1 rounded-full border border-emerald-600">管理者からのお知らせ</span> @endif</td>
                                    </tr>
                                    <tr>
